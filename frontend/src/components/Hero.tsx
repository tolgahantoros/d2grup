import { Award, Gem, Users, Building2, Globe, Play, ArrowRight } from 'lucide-react';
import { motion } from 'motion/react';

// Path to the generated hero image
const HERO_IMAGE_URL = '/src/assets/images/hero_device_1783697684368.jpg';
  ...
  const stats = [
    {
      ...
      icon: <Award className="w-5 h-5 text-white/90" />,
    },
    {
      value: '50+',
      label: 'GLOBAL MARKA',
      icon: <Gem className="w-5 h-5 text-white/90" />,
    },
    {
      value: '150+',
      label: 'UZMAN EKİP',
    ...
    },
    {
      ...
      icon: <Globe className="w-5 h-5 text-white/90" />,
    },
  ];
  ...
  return (
    <section className="relative min-h-screen bg-brand-dark pt-32 pb-16 flex flex-col justify-between overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-12 w-full flex-grow flex flex-col lg:flex-row items-center justify-between gap-12 relative z-10">
        ...
        {/* Pagination Dots (Left Side indicator) */}
        <div className="absolute left-0 top-1/2 -translate-y-1/2 hidden xl:flex flex-col gap-3">
          {[1, 2, 3, 4].map((dot, index) => (
            <div
              key={dot}
              className={`w-1.5 h-1.5 rounded-full transition-all duration-300 ${
                index === 0 ? 'bg-white scale-125' : 'bg-white/20 hover:bg-white/50'
              }`}
            />
          ))}
        </div>
        ...
        <div className="flex-1 text-left xl:pl-8 max-w-2xl">
          ...
          >
            TEKNOLOJİDE <br />
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-white via-white/95 to-white/60">LİDER.</span>
          </motion.h1>
          ...
          >
            <a
              href="#urunler"
              className="flex items-center gap-3 bg-white text-black font-semibold text-xs tracking-widest uppercase py-4.5 px-8 hover:bg-zinc-200 transition-all duration-300"
            >
            ...
            <button
              onClick={() => {
                const target = document.getElementById('tanitim-videosu');
        ...
        <div className="flex-1 flex justify-center items-center relative py-12 lg:py-0 w-full">
          ...
          {/* Interactive Device Render Card */}
          <motion.div
          ...
          >
            ...
            <img
              src={HERO_IMAGE_URL}
              alt="D2 Grup Premium Beauty Aesthetics Device"
              referrerPolicy="no-referrer"
              className="w-full h-full object-cover transition-transform duration-1000 group-hover:scale-105"
            />
          </motion.div>

          {/* Device Model Label Tag */}
        ...
        <div className="absolute right-0 top-1/2 -translate-y-1/2 hidden xl:flex flex-col items-center gap-16">
          <span className="font-sans font-semibold text-[9px] tracking-[0.4em] text-white/40 uppercase rotate-90 origin-center whitespace-nowrap translate-y-4">
            SCROLL
          </span>
      ...
      {/* Metrics Banner Bar (Bottom) */}
      <div className="max-w-7xl mx-auto px-6 md:px-12 w-full mt-12 relative z-10">
        <motion.div
          initial={{ opacity: 0, y: 30 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.7, delay: 0.4 }}
          className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-6 md:gap-8 bg-zinc-950/40 backdrop-blur-md border border-white/10 p-8 md:px-10 rounded-sm relative overflow-hidden group"
        >
          {/* Thin ambient linear scan overlay */}
          <div className="absolute inset-x-0 top-0 h-[1px] bg-gradient-to-r from-transparent via-white/20 to-transparent" />

          {stats.map((stat, i) => (
            <div
              key={stat.label}
              className={`flex flex-col gap-2.5 items-start relative ${
                i > 0 && i !== 2 && i !== 3 ? 'lg:border-l lg:border-white/10 lg:pl-8' : ''
              }`}
            >
