import { ArrowRight, Star, ShieldAlert, BadgeCheck } from 'lucide-react';
import { motion } from 'motion/react';

const COSMETICS_IMAGE_URL = '/src/assets/images/cosmetics_set_1783697729216.jpg';

const BRANDS = [
  { name: 'Lumenis', tagline: 'Energy to Healthcare', logo: 'Lumenis®' },
  { name: 'Fotona', tagline: 'choose perfection', logo: 'Fotona' },
  { name: 'Candela', tagline: 'Science. Results. Trust.', logo: 'CANDELA' },
  { name: 'Sciton', tagline: 'BECAUSE RESULTS MATTER', logo: 'SCITON' },
  { name: 'Inmode', tagline: 'Aesthetic Solutions', logo: 'I N M O D E' },
];

export default function CosmeticsBrands() {
  return (
    <section id="kozmetik" className="bg-white py-24 px-6 md:px-12 border-b border-zinc-100">
      <div className="max-w-7xl mx-auto">
        <div className="flex flex-col lg:flex-row gap-8 items-stretch">
          
          {/* Left Panel: Cosmetics Products (Dark elegant card) */}
          <div className="flex-1 bg-zinc-950 text-white p-10 md:p-14 flex flex-col justify-between rounded-sm relative overflow-hidden group min-h-[580px]">
            ...
            {/* Background image which we generated */}
            <img
              src={COSMETICS_IMAGE_URL}
              alt="D2 Grup Premium Skincare Cosmetics"
            ...
            <div className="relative z-20">
              ...
              <h2 className="font-display font-black text-3xl md:text-4xl tracking-tight text-white uppercase leading-[1.05] max-w-sm mb-6">
                BİLİMSEL FORMÜLLER,<br />GÖZLE GÖRÜLÜR ETKİ.
              </h2>
              <p className="font-sans font-light text-white/70 text-xs leading-relaxed max-w-xs mb-8">
                Hücresel düzeyde yaşlanma karşıtı ve yenileyici etki sunan formüllerimiz, klinik tedavilerin etkinliğini desteklemek üzere tasarlanmıştır.
              </p>
            </div>
          ...
          {/* Right Panel: Brands (Light grey card) */}
          <div id="markalar" className="flex-1 bg-zinc-50 p-10 md:p-14 flex flex-col justify-between rounded-sm border border-zinc-100 min-h-[580px]">
            <div>
              ...
              <h2 className="font-display font-black text-3xl md:text-4xl tracking-tight text-zinc-950 uppercase leading-[1.05] mb-12">
                DÜNYANIN LİDER<br />MARKALARI
              </h2>

              {/* Grid of clean brand listings with hover animations */}
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-8 mb-12">
                {BRANDS.map((brand) => (
                  <div
                    key={brand.name}
                    className="flex flex-col p-6 bg-white border border-zinc-100/80 hover:shadow-md hover:border-zinc-200 transition-all duration-300 rounded-sm group cursor-pointer"
                  >
                ...
                {/* Extra Brand Block */}
                <div className="flex items-center justify-center p-6 bg-dashed border-2 border-zinc-200 rounded-sm hover:bg-zinc-100/50 transition-all cursor-pointer group">
            ...
            <div>
              <a
                href="#markalar-liste"
                className="flex items-center justify-center gap-3 bg-zinc-950 text-white font-semibold text-xs tracking-widest uppercase py-4.5 px-8 hover:bg-zinc-800 transition-all duration-300 w-full sm:w-auto sm:inline-flex"
              >
